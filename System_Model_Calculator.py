...
class system_model:
    def __init__(self, delay, tau1, tau2, dynamic_skew, tbase, gain):
        self.delay = delay
        self.tau1 = tau1
        self.tau2 = tau2
...
def simulate_system_model(model, debug=0):
    ...
    tsens_simulated_2 = tsens_simulated
...
def get_parameter_list(model):
    param_list = []
    param_list.append(model.delay)
    param_list.append(model.tau1)
    param_list.append(model.tau2)
    param_list.append(model.dynamic_skew)
    param_list.append(model.tbase)
    param_list.append(model.gain)
...
def neighbour_generator_function(model=None, max_relative_step_size=0.01):
    ...
    for idx in range(len(parameters_range)):
        if(model == None):
            ...
            model_parameters = get_parameter_list(model)
...
def main():
    ...
    if args.debug:
        ...
        tbase = -19
        gain = -6/3000
        model = system_model(delay, tau1, tau2, dynamic_skew, tbase, gain)
        simulate_data = simulate_system_model(model)
    ...
    if args.debug:
        best_sample = get_parameter_list(sa_process.best_sample)
